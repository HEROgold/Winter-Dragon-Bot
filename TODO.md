...
Add a sherlock-project <https://github.com/sherlock-project/sherlock> watcher with alias support
to notify users of new account creation
...
- [Dockerfile](Dockerfile#L71) runs `CMD ["python", "-m", "winter_dragon"]`, but [src/winter_dragon/**init**.py](src/winter_dragon/__init__.py) is an **empty** package with no `__main__.py`.
