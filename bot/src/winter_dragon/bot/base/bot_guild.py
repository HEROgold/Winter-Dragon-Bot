...
class GuildInitializer(Cog):
    ...
    async def cog_load(self) -> None:
        ...
        if config.get("Main", "support_guild_id") != "!!":
            return
        ...
        config.set("Main", "support_guild_id", str(guild.id))
    ...
    async def _invite_owners(self, guild: Guild) -> None:
        ...
        for owner_id in self.bot.owner_ids:
            ...
            await owner.send(f"Invite to {config.get("Main", "bot_name")}'s official discord server: {self.invite}")
    ...
    async def _create_guild(self) -> Guild:
        guild = await self.bot.create_guild(name=config.get("Main", "bot_name") + " Support Guild")
        config.set("Main", "support_guild_id", str(guild.id))
        return guild
    ...
    async def on_member_join(self, member: Member) -> None:
        ...
        if member.id not in self.bot.owner_ids:
            ...
            return
        if member.guild.id != config.getint("Main", "support_guild_id"):
