...
class OverwriteCreate(AuditEvent, action=AuditLogAction.overwrite_create):
    """Handle overwrite create events."""

    @override
    async def handle(self) -> None:
        # https://discordpy.readthedocs.io/en/stable/api.html?highlight=auditlogentry#discord.AuditLogAction.overwrite_create
        self.logger.debug(f"on overwrite_create: {self.entry.guild=}, {self.entry=}")
