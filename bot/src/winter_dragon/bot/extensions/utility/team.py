...
from winter_dragon.bot._types.aliases import GTPChannel
...
class Team(GroupCog):
    ...
    async def delete_empty_team_channels(self) -> None:
        ...
        with self.session as session:
            channels = session.exec(select(Channel).where(Channel.type == TEAM_VOICE)).all()
    ...
    def get_teams_category(self, guild: Guild | None) -> CategoryChannel | None:
        ...
        with self.session as session:
            if channel := session.exec(select(Channel)).where(
