...
from winter_dragon.database.tables import Game, NhieQuestion, Suggestion
...
class NeverHaveIEver(GroupCog):
    ...
    def set_default_data(self) -> None:
        with self.session as session:
            questions = session.exec(select(NhieQuestion)).all()
            if len(questions) > 0:
    ...
    def get_questions(self) -> tuple[Literal[0], Sequence[NhieQuestion]]:
        with self.session as session:
            questions = session.exec(select(NhieQuestion)).all()
            game_id = 0
    ...
    async def slash_nhie_add_verified(self, interaction:discord.Interaction) -> None:
        with self.session as session:
            result = session.exec(select(Suggestion).where(Suggestion.type == NHIE, Suggestion.is_verified))
