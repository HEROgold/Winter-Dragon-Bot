...
class CombinedCommand:
    ...
    def __init__(self, dc_command: DcCommand) -> None:
        ...
        with Session(engine) as session:
            self.db_command = session.exec(select(DbCommand).where(DbCommand.qual_name == dc_command.qualified_name)).first()
...
class CommandControl(GroupCog):
    ...
    async def slash_command_disable(self, interaction: discord.Interaction, command: str) -> None:
        """CommandControl."""
        with self.session as session:
            cmd = session.exec(select(GuildCommands).join(DbCommand).where(DbCommand.qual_name == command)).first()
...
#             db_command = session.exec(select(CommandControl).filter_by(guild_id=guild_id, command_name=dc_command.qualified_name)).first()
...
#     with Session() as session:
#         db_command = session.exec(select(CommandControl).filter_by(guild_id=guild_id, command_name=command_name)).first()
#         if db_command and not db_command.enabled:
