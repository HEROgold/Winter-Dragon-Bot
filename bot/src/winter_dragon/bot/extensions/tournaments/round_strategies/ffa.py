...
class FFAStrategy(TournamentStrategy):
    ...
    REQUIRED_TEAM_SIZE = 3  # Minimum teams for FFA
    ...
    async def generate_matches(self, teams: Sequence[TournamentTeam]) -> Sequence[TournamentMatch]:
        ...
        for i in range(0, len(teams), group_size):
            ...
            if len(group_teams) >= self.REQUIRED_TEAM_SIZE:  # Need at least 3 for FFA
    ...
    async def advance_to_next_round(self, completed_matches: Sequence[TournamentMatch]) -> Sequence[TournamentMatch]:
        ...
        for i in range(0, len(winners), group_size):
            ...
            if len(group_winners) >= self.REQUIRED_TEAM_SIZE:
                match = TournamentMatch(
                    tournament_id=self.tournament.id,
