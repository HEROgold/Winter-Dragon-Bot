...
class SingleEliminationStrategy(TournamentStrategy):
    ...
    REQUIRED_TEAM_SIZE = 2  # Minimum teams for single elimination
    ...
    async def generate_matches(self, teams: Sequence[TournamentTeam]) -> Sequence[TournamentMatch]:
        ...
        shuffled_teams: list[TournamentTeam] = list(teams)
        import random
    ...
    async def advance_to_next_round(self, completed_matches: Sequence[TournamentMatch]) -> Sequence[TournamentMatch]:
        ...
        if len(winners) < self.REQUIRED_TEAM_SIZE:
            return []

        next_round_matches = []
        next_round = current_round + 1

        for i in range(0, len(winners), self.REQUIRED_TEAM_SIZE):
            if i + 1 < len(winners):
                match = TournamentMatch(
                    tournament_id=self.tournament.id,
                    team1_id=winners[i],
    ...
    def get_standings(self, teams: Sequence[TournamentTeam], matches: Sequence[TournamentMatch]) -> Sequence[TournamentTeam]:
        ...
        # Add winner first
        if winner and (winner_team := next((t for t in teams if t.id == winner), None)):
