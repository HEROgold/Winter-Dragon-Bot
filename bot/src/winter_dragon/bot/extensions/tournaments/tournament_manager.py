...
from winter_dragon.bot.constants import DISCORD_AUTOCOMPLETE_LIMIT
...
from .round_strategies.double_elimination import DoubleEliminationStrategy
from .round_strategies.ffa import FFAStrategy
from .round_strategies.round_robin import RoundRobinStrategy
from .round_strategies.single_elimination import SingleEliminationStrategy
...
if TYPE_CHECKING:
    ...
    from winter_dragon.bot.extensions.tournaments.round_strategies.base import TournamentStrategy
...
# TODO: use utility/team.py functionality to help with tournaments. (modularity)
class TournamentManager(GroupCog):
    """Tournament management system for Discord servers."""

    max_displayed_matches = Config(5)


    def __init__(self, **kwargs: Unpack[BotKwarg]) -> None:
        """Initialize the tournament manager."""
        super().__init__(**kwargs)
        self.strategies: dict[TournamentType, type[TournamentStrategy]] = {
    ...
    async def register_tournament(
        ...
        if not interaction.user:
            await interaction.response.send_message("You must be logged in to register for a tournament.", ephemeral=True)
            return
        ...
        if not tournament.id:
            self.logger.warning(f"Failed to find tournament: {tournament=} {tournament_name=} in guild {guild.id=}.")
            tournament = None
    ...
    async def list_tournaments(self, interaction: discord.Interaction) -> None:
        ...
        for tournament in tournaments:
            ...
            embed.add_field(
                ...
                value=dedent(
                    f"""Type: {tournament.tournament_type.value.replace('_', ' ').title()}
                    "Status: {tournament.status.value.replace('_', ' ').title()}
    ...
    async def assign_teams(
        ...
        if not (
            ...
            or await self._is_tournament_creator(interaction.user.id, tournament_name, interaction.guild.id)
        ):
    ...
    # Use command groups to organize commands, not `-`
    ...
    async def create_channels(
        ...
        try:
            ...
            if not category:
                await interaction.followup.send("Failed to create or find tournament category.", ephemeral=True)
                return
            ...
            self.logger.exception(f"Error creating channels: {e}")
    ...
    async def _auto_assign_teams(self, tournament: Tournament, players: Sequence[TournamentPlayer]) -> None:
    ...
    async def _create_team_channels(
        ...
        team_members: list[discord.Member] = []
        ...
        # TODO: Check if we have a way or removing these after tournamen is finished.
        role = None
        if use_roles:
    ...
    async def show_scoreboard(
        ...
        if tournament.tournament_type == TournamentType.ROUND_ROBIN:
            ...
            # TODO: Move the standings message generation to the TournamentStrategy
            # TODO: Embed.add_field() per team, not in one message.
            standings_text = ""
            for i, team in enumerate(standings, 1):
                standings_text += f"{trophy_emojis.get(i, f"{i}.")} **{team.name}** - {team.points} pts ({team.wins}W-{team.draws}D-{team.losses}L)\n"
            ...
            for i, team in enumerate(standings, 1):
                if team.status.value == "eliminated":
                    ...
                    status = "⚪"
                standings_text += f"{trophy_emojis.get(i, f"{i}.")} {status} **{team.name}** ({team.wins}W-{team.losses}L)\n"
    ...
    async def start_tournament(
        ...
        try:
            ...
            if matches:
                match_list = ""
                for match in matches[:self.max_displayed_matches]:
                ...
                if len(matches) > self.max_displayed_matches:
                    match_list += f"... and {len(matches) - self.max_displayed_matches} more matches"
            ...
            self.logger.exception(f"Error starting tournament: {e}")
            await interaction.followup.send(
