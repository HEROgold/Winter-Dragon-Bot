...
from winter_dragon.database.tables import Role as DbRole
...
class AutoAssign(GroupCog):
    ...
    async def slash_assign_show(self, interaction: discord.Interaction) -> None:
        with self.session as session:
            aar = session.exec(select(AutoAssignRole).where(AutoAssignRole.guild_id == interaction.guild.id)).first()
    ...
    async def remove_all_roles(self, interaction: discord.Interaction) -> None:
        ...
        with self.session as session:
            for auto_assign in session.exec(select(AutoAssignRole).where(AutoAssignRole.guild_id == interaction.guild.id)).all():
                session.delete(auto_assign)
            session.commit()
