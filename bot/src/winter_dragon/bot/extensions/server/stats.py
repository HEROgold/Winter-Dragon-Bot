...
class Stats(GroupCog):
    ...
    async def update(self) -> None:
        ...
        for guild in guilds:
            ...
            peak_channel, guild_channel, bot_channel, user_channel, online_channel = await self.get_guild_stats_channels(guild)
            self.logger.debug(
        ...
        with self.session as session:
            ...
            for channel in channels:
                if channel.id is None:
                    self.logger.warning(f"Channel id is None: {channel=}")
                    continue
