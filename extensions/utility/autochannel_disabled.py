...
async def on_voice_state_update(
    ...
    category, text, voice = get_guild_auto_channels(after.channel.guild)
    automatic_channels = [category.id, text.id, voice.id]
...
def get_guild_auto_channels(guild: discord.Guild) -> tuple[Channel, Channel, Channel]:
    with Session(engine) as session:
        # Should be three.
        for channel in session.query(Channel).where(
