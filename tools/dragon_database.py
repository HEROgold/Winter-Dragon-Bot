...
class Database():
    ...
    def __get_database__(self, database_name:str) -> mdb.Database:
        mc = self.get_client()
        # self.logger.debug(f"Getting database: {database_name}")
        return mc.get_database(database_name)

    def __get_collection__(self, collection_name:str) -> mcol.Collection:
        database = self.__get_database__(self.target_database)
    ...
    def get_data(self, collection_name:str) -> dict:
        collection = self.__get_collection__(collection_name)
    ...
    def set_data(self, collection_name:str, data:dict) -> None:
        collection = self.__get_collection__(collection_name)
        try:
