...
from confkit.data_types import Time

# TODO: This file is NOT needed with the bot's new EventSystem.
# Move related events to relevant cogs.
...
class DatabaseManager(Cog):
    ...
    database_update_interval = Config(Time(3600))  # 1 hour in seconds

    async def cog_load(self) -> None:
        """Load the cog."""
        await super().cog_load()
        # Configure loop interval from config
        self.update.change_interval(hours=self.database_update_interval)
        self.update.start()
